...
public interface RecordPatientRepository extends ReactiveCrudRepository<RecordPatient, Long>{   
    ...
    @Query("SELECT rp.* FROM record_patient rp WHERE rp.card_patient_id = :idPatient AND (( rp.date_record >= :from) AND ( rp.date_record <= :to))" )
    Flux<RecordPatient> findByParamByPatient( Long idPatient, LocalDateTime from, LocalDateTime to);
    ...
    Flux<RecordPatient> findByParamByCardPatient( Long idCardPatient, LocalDateTime from, LocalDateTime to);
}
